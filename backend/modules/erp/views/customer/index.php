...
<div class="customer-index">
    <p>
        <?= Html::a('创建会员', ['edit'], ['class' => 'btn btn-outline-success']) ?>
    </p>
        ...
        'columns' => [
            ...
            [
                ...
                'value' => function ($data) {
                    ...
                    $url = Yii::$app->urlManager->createUrl(['/erp/customer/view', 'customer_id' => $data['customer_id']]);
                    $html .= Html::a('<i class="glyphicon glyphicon-eye-open"></i>', $url, ['data-title' => '查看']);
                    $url = Yii::$app->urlManager->createUrl(['/erp/customer/edit', 'customer_id' => $data['customer_id']]);
                    ...
                    if ($data['status'] == 1) {
                        $html .= Html::a('<i class="glyphicon glyphicon-ban-circle"></i>', 'javascript:void(0)', ['title' => '禁用', 'class' => 'set-status-btn', 'data-id' => $data['customer_id'], 'data-status' => 0, 'data-url' => $url]);
                    } else {
                        $html .= Html::a('<i class="glyphicon glyphicon-ok"></i>', 'javascript:void(0)', ['title' => '启用', 'class' => 'set-status-btn', 'data-id' => $data['customer_id'], 'data-status' => 1, 'data-url' => $url]);
